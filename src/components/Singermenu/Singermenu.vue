<template>
  <transition-group name="fade" >
    <div v-show="showFlag" class="musicmenu-wrapper" key="musicmenu">
      ...
      <div ref="songmenuWrapper" class="list-wrapper">
        <div>
          <header>
            <div class="menu-user">
              <div class="user-img" >
            ...
            <div class="background">
              <img v-lazy="coverImgUrl" alt="" width="100%" height="100%">
            </div>
          </header>
          <div class="menu-list">
...
export default{
  ...
  methods: {
    ...
    setmusiclist(item) {
        console.log(item);
        console.log(item.artist.briefDesc)
    //   this.muiscdesc = item;
    //   console.log(this.muiscdesc);
    //   this._get(item);
      this.coverImgUrl = item.artist.img1v1Url;
      this.name = item.artist.name;
      this.description = item.artist.briefDesc;
      this.musiclist = item.hotSongs;
    //   this.creator = item.creator;
      this.id = item.artist.id;
      this.trackCount = item.hotSongs.length;
      this._initScroll();
    },
    _get(item) {
      axios.get(api.getSongs(item.id)).then((res)=>{
    ...
    _initScroll() {
      if (!this.songScroll) {
        this.songScroll = new BScroll(this.$refs.songmenuWrapper, {
          probeType: 3,
          click: true
        });
